...
for (idstanice,cas) in schema:
    print(idstanice,cas)
    starttime = datetime.combine(datetime(year=2021, month=8, day=2),cas)
    iterace = heapdict.heapdict()
    iterace[(idstanice,frozenset([idstanice]))] = Trasa(starttime,idstanice,0,0.0,set(),{},countVykon,0)
    x = 0
    ...
    if (idstanice,cas) not in vysledky:
        vysledky[(idstanice,cas)] = {}

    if len(iterace):
        (x,trasa) = iterace.popitem()
        vysledky[(idstanice,cas)] = trasa.vykon
    else:
        vysledky[(idstanice,cas)] = 0



with open(f'C:\\Railtour\\hrany\\vykony_schema.pickle', 'wb') as handle:
