...
def doIterace():
    ...
    (body,idstanice) = heapq.heappop(poradi)
    trasa = random.choice(iterace[(body,idstanice)][1])
    print(body)
    ...
    for cil,hrany in graf[idstanice].items():
        if (cil != OLOMOUC) and (cil not in visited) and ((trasa.cas>=BLOCK_DO) or (cil not in TEMP_BLOCK)) and len(hrany):
            ...
            if ((new_values['body'],cil) not in iterace) or (iterace[(new_values['body'],cil)][0] < new_trasa.vykon):     
                iterace[(new_values['body'],cil)] = (new_trasa.vykon,[new_trasa])
            if ((new_values['body'],cil) in iterace) and (iterace[(new_values['body'],cil)][0] == new_trasa.vykon):     
                iterace[(new_values['body'],cil)][1].append(new_trasa)
            if (new_values['body'],cil) not in poradi:
                heapq.heappush(poradi,(new_values['body'],cil))
    ...
    and (trasa.cas<=SPANEK_DO) and (trasa.cas>=SPANEK_OD):
        ...
        new_body = trasa.body + 5
        new_trasa = Trasa(new_cas,trasa.idstart,new_body,\
                                                        trasa.km,new_kraje,\
                                                            new_hrany,countVykon,new_inday,0)
        if ((new_body,idstanice) not in iterace) or (iterace[(new_body,idstanice)][0] < new_trasa.vykon):     
            iterace[(new_body,idstanice)] = (new_trasa.vykon,[new_trasa])
        if ((new_body,idstanice) in iterace) and (iterace[(new_body,idstanice)][0] == new_trasa.vykon):     
            iterace[(new_body,idstanice)][1].append(new_trasa)
        if (new_body,idstanice) not in poradi:
            heapq.heappush(poradi,(new_body,idstanice)) 
...
countVykon = countVykon2R
koef_unavy = UNAVA

iterace = {}
iterace[(0,START)] = (0,[Trasa(starttime,START,0,0.0,set(),{},countVykon,0,0)])
