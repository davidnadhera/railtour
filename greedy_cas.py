...
def doIterace():
    ...
    (cas,idstanice) = heapq.heappop(poradi)
    trasa = random.choice(iterace[(cas,idstanice)][1])
    print(cas)
    ...
    for cil,hrany in graf[idstanice].items():
        if (cil != OLOMOUC) and (cil not in visited) and ((trasa.cas>=BLOCK_DO) or (cil not in TEMP_BLOCK)) and len(hrany):
            ...
            if ((new_cas,cil) not in iterace) or (iterace[(new_cas,cil)][0] < new_trasa.vykon):     
                iterace[(new_cas,cil)] = (new_trasa.vykon,[new_trasa])
            if ((new_cas,cil) in iterace) and (iterace[(new_cas,cil)][0] == new_trasa.vykon):     
                iterace[(new_cas,cil)][1].append(new_trasa)
            if (new_cas,cil) not in poradi:
                heapq.heappush(poradi,(new_cas,cil))
    ...
    and (trasa.cas<=SPANEK_DO) and (trasa.cas>=SPANEK_OD):
        ...
        if ((new_cas,idstanice) not in iterace) or (iterace[(new_cas,idstanice)][0] < new_trasa.vykon):     
            iterace[(new_cas,idstanice)] = (new_trasa.vykon,[new_trasa])
        if ((new_cas,idstanice) in iterace) and (iterace[(new_cas,idstanice)][0] == new_trasa.vykon):     
            iterace[(new_cas,idstanice)][1].append(new_trasa)
        if (new_cas,idstanice) not in poradi:
            heapq.heappush(poradi,(new_cas,idstanice)) 
...
countVykon = countVykon1R
koef_unavy = UNAVA

iterace = {}
iterace[(starttime,START)] = (0,[Trasa(starttime,START,0,0.0,set(),{},countVykon,0,0)])

poradi = [(starttime,START)]
