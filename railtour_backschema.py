...
def countVykon1(trasa):
    doba = CIL_DO-trasa.cas
    doba = doba.total_seconds()
    penale = max(0,(trasa.km-kmsec*doba)*PENALE)
 
    if doba:
        trasa.vykon = (trasa.body-penale+trasa.predict_vykon)/(doba)*60.0*60.0 
    else:
        trasa.vykon = 0.0
...
def doIterace(iterace,x):
    ...
    while len(iterace) and (pocet<MAX_V_ITERACI):
        ...
        if (idstanice,trasa.cas.time()) not in schema:
            print(idstanice,trasa.cas.time())     
        for cil,info in schema[(idstanice,trasa.cas.time())].items():
            ...
            and ((cil != SPANEK) or ((trasa.cas>=SPANEK_OD+timedelta(hours=6)) and (trasa.cas<=SPANEK_DO+timedelta(hours=6)))):
                (doba,hrana,vykon) = info
                if vykon<LIMIT_VYKON:
                    continue
                new_cas = trasa.cas - doba
                if (new_cas < starttime):
                    continue
                in_it += 1
                ...
                new_trasa = Trasa(new_cas,idstanice,new_values['body'],trasa.km + hrana.km,\
                ...
                (doba,hrana,vykon) = info
                new_cas = trasa.cas - doba
                if (new_cas > starttime+timedelta(hours=4)) or (new_cas < starttime):
                    continue
                new_values = getNewValues(new_cas,trasa.cas,trasa.kraje,trasa.inday,trasa.body,cil)
                new_hrany = trasa.hrany.copy()
                new_hrany[cil] = (hrana,new_values['premie'],0)
                vysledky.append(Trasa(new_cas,idstanice,new_values['body'],trasa.km + hrana.km,\
...
with open('C:\\Railtour\\hrany\\backschema_full.pickle', 'rb') as handle:
    schema = pickle.load(handle)
with open('C:\\Railtour\\hrany\\schema_full.pickle', 'rb') as handle:
    fwschema = pickle.load(handle)
...
iterace[(START,frozenset([START]))] = Trasa(CIL_DO-timedelta(minutes=3),OLOMOUC,0,0.0,set(),{},countVykon,0,0)
x = 0
